...
pub fn hash_to_try_and_increment(message: &[u8]) -> Option<G1> {
    let mut c = 0..255;
    ...
    let point = c.find_map(|ctr| {
        v[position] = ctr;
        let hash = &calculate_sha256(&v)[0..32];
        ...
        module_hash
            .to_big_endian(&mut s)
            .ok()
            .and_then(|_| arbitrary_string_to_g1(&s).or(None))
    });
...
fn arbitrary_string_to_g1(data: &[u8]) -> Option<G1> {
    let mut v = vec![0x02];
    v.extend(data);
